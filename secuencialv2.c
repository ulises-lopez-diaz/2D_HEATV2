...
int main(int argc, char **argv)
{
	...
	while (err_ss_max>=tol_ss || err_ss_min>=tol_ss){		
		...
		for (i=1; i<Nx+2; i++){// imprimir variables enteras asociadas con Tss y T 
	...
	sec = end.tv_sec-start.tv_sec; 
	nsec = (end.tv_nsec - start.tv_nsec)/MIL_MILLONES; // convertido a segundos
	fd_time = sec + nsec;
	...
	FILE *finite_differences_txt;
		

	// imprime matriz T 
	int step;
	
	finite_differences_txt = fopen("finite_differences_txt.txt", "w+");
	
	for (step=0; step<k; step++){
		for (i=0; i<Nx+2; i++){
			for (j=0; j<Ny+2; j++){
				fprintf(finite_differences_txt, "%f\t", T[i][j][step]);
			}
			fprintf(finite_differences_txt, "\n");
		}
	}
	
	printf("... archivo de salida creados con éxito. Programa terminado. \n\n");
